...
class Trainer:
    ...
    def train(self, epoch, log_interval):
        ...
        for batch_idx, (data, _) in enumerate(self.train_loader):
            ...
            target = data
            train_loss = self.loss_function(output_data, target, mu, logvar)
            train_loss.backward()
            self.optimizer.step()
    ...
    def test(self, epoch, batch_size, log_interval):
        ...
        for batch_idx, (data, _) in enumerate(self.test_loader):
            ...
            output_data, mu, logvar = self.model(data)
            target = data
            test_loss = self.loss_function(output_data, data, mu, logvar)
    ...
    def plot_generated(self, epoch, batch_size):
        batch, _ = next(iter(self.test_loader))
        grid_real = vutils.make_grid(batch, nrow=int(batch_size**0.5))
...
def show(img, epoch):
    ...
    plt.imsave(f"vae_images_epoch_{epoch}.png", np.transpose(npimg, (1,2,0)))
